...
class Environment:
    ...
    def load(self):
        ...
        object_start_orientation_q = p.getQuaternionFromEuler(
            config.object_start_orientation_e
        )
        object_model = p.loadURDF(
            "ycb_assets/006_mustard_bottle.urdf",
...
def run_simulation_environment(args, env_connection, logger):
    ...
    robot.move(
        env,
        robot.ee_start_position,
        robot.ee_start_orientation_e,
        gripper_open=True,
        is_trajectory=False,
    )
    ...
    while True:
        ...
        if env_connection.poll():
            ...
            if env_connection_received[0] == CAPTURE_IMAGES:
                ...
                head_camera_position, head_camera_orientation_q = (
                ...
                wrist_camera_position, wrist_camera_orientation_q = (
                    robot.get_camera_image(
                        "wrist",
                        env,
                        save_camera_image=True,
                ...
                env_connection_message = (
                    OK + "Finished capturing head camera image!" + ENDC
                )
                env_connection.send(
                    [
                ...
                for bounding_cube_world_coordinates in bounding_cubes_world_coordinates:
                    ...
                    p.addUserDebugLine(
                        bounding_cube_world_coordinates[2],
                        bounding_cube_world_coordinates[3],
                        [0, 1, 0],
                        lifeTime=0,
                    )
                    p.addUserDebugLine(
                    ...
                    p.addUserDebugLine(
                        ...
                        bounding_cube_world_coordinates[6],
                        [0, 1, 0],
                        lifeTime=0,
                    )
                    p.addUserDebugLine(
                        bounding_cube_world_coordinates[6],
                        bounding_cube_world_coordinates[7],
                        [0, 1, 0],
                        lifeTime=0,
                    )
                    p.addUserDebugLine(
                        bounding_cube_world_coordinates[7],
                        bounding_cube_world_coordinates[8],
                        [0, 1, 0],
                        lifeTime=0,
                    )
                    p.addUserDebugLine(
                    ...
                    p.addUserDebugPoints(
                        bounding_cube_world_coordinates,
                        [[0, 1, 0]] * len(bounding_cube_world_coordinates),
                        pointSize=5,
                        lifeTime=0,
                    )

                env_connection_message = (
                    OK + "Finished adding bounding cubes to the environment!" + ENDC
                )
                env_connection.send([env_connection_message])
                ...
                p.addUserDebugPoints(
                    trajectory_points,
                    [[0, 1, 1]] * len(trajectory_points),
                    pointSize=5,
                    lifeTime=0,
                )

                logger.info(
                    OK + "Finished adding trajectory points to the environment!" + ENDC
                )
                ...
                for point in trajectory:
                    robot.move(
                        env,
                        point[:3],
                        np.array(robot.ee_start_orientation_e)
                        + np.array([0, 0, point[3]]),
                ...
                ee_current_position = p.getLinkState(
                    robot.id, robot.ee_index, computeForwardKinematics=True
                )[0]
                ee_current_orientation_q = p.getLinkState(
                    robot.id, robot.ee_index, computeForwardKinematics=True
                )[1]
                ee_current_orientation_e = p.getEulerFromQuaternion(
                    ee_current_orientation_q
                )

                robot.move(
                    env,
                    ee_current_position,
                    ee_current_orientation_e,
                    gripper_open=True,
                    is_trajectory=False,
                )
                ...
                env_connection_message = (
                    OK + "Finished executing all generated trajectories!" + ENDC
                )
                env_connection.send([env_connection_message])
