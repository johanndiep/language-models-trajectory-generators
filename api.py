...
class API:
    ...
    def __init__(
        self, args, main_connection, logger, langsam_model, xmem_model, device
    ):
    ...
    def detect_object(self, segmentation_text):
        ...
        masks = utils.get_segmentation_mask(
            model_predictions, config.segmentation_threshold
        )

        bounding_cubes_world_coordinates, bounding_cubes_orientations = (
            utils.get_bounding_cube_from_point_cloud(
                rgb_image_head,
                masks,
                depth_array,
        ...
        self.logger.info(
            PROGRESS + "Adding bounding cubes to the environment..." + ENDC
        )
        self.main_connection.send(
            [ADD_BOUNDING_CUBES, bounding_cubes_world_coordinates]
        )
        [env_connection_message] = self.main_connection.recv()
        self.logger.info(env_connection_message)

        for i, bounding_cube_world_coordinates in enumerate(
            bounding_cubes_world_coordinates
        ):

            bounding_cube_world_coordinates[4][2] -= config.depth_offset

            object_width = np.around(
                np.linalg.norm(
                    bounding_cube_world_coordinates[1]
                    - bounding_cube_world_coordinates[0]
                ),
                3,
            )
            object_length = np.around(
                np.linalg.norm(
                    bounding_cube_world_coordinates[2]
                    - bounding_cube_world_coordinates[1]
                ),
                3,
            )
            object_height = np.around(
                np.linalg.norm(
                    bounding_cube_world_coordinates[5]
                    - bounding_cube_world_coordinates[0]
                ),
                3,
            )
            ...
            print(
                "Position of " + segmentation_texts[i] + ":",
                list(np.around(bounding_cube_world_coordinates[4], 3)),
            )
            ...
            if object_width < object_length:
                print(
                    "Orientation along shorter side (width):",
                    np.around(bounding_cubes_orientations[i][0], 3),
                )
                print(
                    "Orientation along longer side (length):",
                    np.around(bounding_cubes_orientations[i][1], 3),
                    "\n",
                )
            else:
                print(
                    "Orientation along shorter side (length):",
                    np.around(bounding_cubes_orientations[i][1], 3),
                )
                print(
                    "Orientation along longer side (width):",
                    np.around(bounding_cubes_orientations[i][0], 3),
                    "\n",
                )
    ...
    def execute_trajectory(self, trajectory):
        ...
        self.logger.info(
            PROGRESS + "Adding trajectory points to the environment..." + ENDC
        )
    ...
    def task_completed(self):
        ...
        if self.attempted_task:
            ...
            self.logger.info(
                PROGRESS + "Waiting to execute all generated trajectories..." + ENDC
            )
            ...
            masks = models.get_xmem_output(
                self.xmem_model, self.device, self.trajectory_length
            )
            ...
            for object in range(1, num_objects + 1):
                ...
                for i, mask in enumerate(masks):
                    ...
                    rgb_image = Image.open(
                        config.rgb_image_trajectory_path.format(
                            step=i * config.xmem_output_every
                        )
                    ).convert("RGB")
                    depth_image = Image.open(
                        config.depth_image_trajectory_path.format(
                            step=i * config.xmem_output_every
                        )
                    ).convert("L")
                    ...
                    if len(bounding_cubes) == 0:
                        ...
                        orientation = (
                            np.mod(orientation + math.pi, 2 * math.pi) - math.pi
                        )
                        ...
                        if i == 0:
                            ...
                            previous_orientation = object_orientations[
                                i - 1 - idx_offset
                            ]
                            possible_orientations = np.array(
                                [
                                    np.mod(
                                        orientation + i * math.pi / 2 + math.pi,
                                        2 * math.pi,
                                    )
                                    - math.pi
                                    for i in range(4)
                            ...
                            circular_difference = np.minimum(
                                np.abs(possible_orientations - previous_orientation),
                                2 * math.pi
                                - np.abs(possible_orientations - previous_orientation),
                            )
                ...
                new_prompt += (
                    self.segmentation_texts[object - 1]
                    + " trajectory positions and orientations:\n"
                )
                ...
                new_prompt += (
                    str(
                        np.around(
                            [
                                position
                                for p, position in enumerate(object_positions)
                                if p % config.xmem_lm_input_every == 0
                            ],
                            3,
                        )
                    )
                ...
                new_prompt += (
                    str(
                        np.around(
                            [
                                orientation
                                for o, orientation in enumerate(object_orientations)
                                if o % config.xmem_lm_input_every == 0
                            ],
                            3,
                        )
                    )
            ...
            self.logger.info(PROGRESS + "Generating LLM output..." + ENDC)
            '''
            ...
            '''
            messages = models.get_mistral_output(
                self.args.language_model,
                new_prompt,
                messages,
                "system",
            ...
            self.logger.info(OK + "Finished generating LLM output!" + ENDC)
